...
evaluation source. LeWM-derived files include `module.py`, `jepa.py`,
`train.py`, `eval.py`, `utils.py`, and their Hydra configurations. The
applicable upstream MIT notice is preserved in
`third_party/LeWM-MIT-LICENSE.txt`.

Third-party datasets, CLEAR-LeWM, simulator assets, and model checkpoints are
not relicensed by this repository. Consult each artifact's own license and
...
The project website vendors Three.js 0.185.1 and OrbitControls under
the MIT License. Its unmodified license text is preserved at
